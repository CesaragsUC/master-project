...
public class MongoRepository<TEntity> : IMongoRepository<TEntity> where TEntity : class
{
    ...
    {
        var client = _dbContext.Database.Client; 
        var settings = client.Settings; 
        var databaseName = _dbContext.Database.DatabaseNamespace.DatabaseName;

        return $"Host: {settings.Server.Host}, Port: {settings.Server.Port}, Database: {databaseName}";
    }
